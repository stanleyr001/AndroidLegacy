...
public class ViewOptionsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        nitView();
        if (toolbar != null) {
            toolbar.setTitle(getString(R.string.toolbarViewOptionsTitle));
        ...
        if (networkInfo != null && networkInfo.isConnected()) {
            ...
            optionsMap = topicsController.getOptionsIDMap(GID, isPrivate, authKey);
            ListView optionsListView = (ListView) findViewById(R.id.view_options_listview);

            if (optionsMap.size() != 0) {

                String[] optionKeys = (String[]) optionsMap.keySet().toArray(new String[optionsMap.size()]);
                String[] optionValues = (String[]) optionsMap.values().toArray(new String[optionsMap.size()]);

                OptionsAdapter optionsAdapter = new OptionsAdapter(ViewOptionsActivity.this, optionKeys, optionValues);

                optionsListView.setAdapter(optionsAdapter);
                Log.d("OPTIONLISTVIEW", "populated successfully");
            }
        }
    }
    ...
    private void nitView() {
        ...
        navigationDrawerAdapter = new ArrayAdapter<>(ViewOptionsActivity.this,
